...
The baseline uses simpler features than optimized (no embeddings, no advanced patterns).
"""
...
# ═════════════════════════════════════════════════════════════════════════════
# REGEX PATTERNS — Binary signals for algorithmic concepts
# ═════════════════════════════════════════════════════════════════════════════
# Each key maps to a regex pattern. A match → feature value 1, else 0.
# This is a subset compared to optimized (which has 50+ patterns).
CODE_PATTERNS = {
    # ─── Math / Number theory ───
...
def extract_code_features(source_code: str) -> dict:
    """
    Extract binary features from source_code.
    Each pattern returns 1 if found, 0 otherwise.
    These binary signals are simple but effective for algorithm detection.
    """
    # ─── Handle missing/invalid source code ───
...
def extract_meta_features(row: pd.Series) -> dict:
    ...
    # ─── Time limit (extract number from text like "2 seconds") ───
    time_limit = 0.0
...
def build_text_input(row: pd.Series) -> str:
    ...
    Null fields are replaced with an empty string.
    
    Concatenation order:
    description → input spec → output spec → notes → source code
    """
    fields = [
...
def build_feature_matrix(df: pd.DataFrame) -> tuple[np.ndarray, list[str]]:
    ...
    1. Meta-features (5 numerical): difficulty, time_limit, desc_len, code_len, input_spec_len
    2. Code features (~38 binary): regex pattern matches
    
    Returns:
        - X_meta: np.ndarray of shape (n_samples, n_features)
        - feature_names: list of column names for reference
    """
    # ─── Extract features for each row ───
    rows = []
    for _, row in df.iterrows():
        # Get meta-features (5 numerical)
        meta = extract_meta_features(row)
        # Get code features (~38 binary)
    ...
    # ─── Return as numpy array with feature names ───
...
if __name__ == "__main__":
    ...
    print("\n=== Code features (active patterns only) ===")
    ...
    print(f"Features ({len(names)} total): {names}")